...
const DEFAULT_CONFIG = `{
  ...
  "refiner": {
    "provider": "openai",
    "model": "gpt-4o-mini",
    "systemPrompt": "You are a professional translator and localization expert. Refine the following translations to ensure they are accurate, natural, and culturally appropriate for the target audience. Pay special attention to idiomatic expressions, context, and tone. Provide the refined translations in the same format as the input."
  }
}
`;
  ...
  command
    ...
    .action(async (options) => {
      ...
      if (existsSync(configPath) && !options.force) {
        logger.error(
          "stryngz.config.json already exists. Use --force to overwrite.",
        );
        process.exit(1);
      }
      ...
      try {
        ...
        logger.info(
          "  1. Edit stryngz.config.json to match your project structure",
        );
        logger.info(
          "  2. Configure your translation providers in the 'translations' array",
        );
        ...
        logger.info(
          "     - OPENAI_API_KEY (or other provider key for AI refiner)",
        );
